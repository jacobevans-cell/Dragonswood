...
function playAdventureIdentity(stateName='play'){
  ...
  if(identity.appearance?.v5)window.DWRPG.renderV5Character(hero,identity.appearance,stateName,{prefix:'../',version:'56.28-v5.3.4',alt:identity.appearanceName});else{const animated=identity.appearance?.[`${stateName}Art`]||identity.appearance?.attackArt||identity.heroArt;hero.src=animated}hero.classList.remove('live-adventurer-action');void hero.offsetWidth;hero.classList.add('live-adventurer-action');adventurePetActor?.play(stateName,stateName==='celebrate'?1400:1000);setTimeout(()=>{if(hero.isConnected){if(identity.appearance?.v5)window.DWRPG.renderV5Character(hero,identity.appearance,'idle',{prefix:'../',version:'56.28-v5.3.4',alt:identity.appearanceName});else hero.src=identity.heroArt;hero.classList.remove('live-adventurer-action')}},1100);
}
function mountAdventureIdentity(){
  ...
  if(identity.appearance?.v5)window.DWRPG.renderV5Character(hero,identity.appearance,'idle',{prefix:'../',version:'56.28-v5.3.4',alt:identity.appearanceName});else hero.onerror=()=>{if(hero.getAttribute('src')!==identity.staticHeroArt)hero.src=identity.staticHeroArt};
