"""Build verified chapter boundaries for the classroom Wild Robot Escapes PDF edition."""
...
SOURCE = ROOT / "class-library" / "books" / "wild-robot-2-escapes.pdf"
OUTPUT = ROOT / "class-library" / "books" / "wild-robot-2" / "meta.js"
FINAL_STORY_PAGE = 215
CHAPTER_COUNT = 90
...
def main() -> None:
    ...
    for page_number, page in enumerate(reader.pages, 1):
        ...
        if chapter_number == 70:
            title = "The March Through the City"
    ...
    if [number for number, _, _ in starts] != list(range(1, CHAPTER_COUNT + 1)):
        raise RuntimeError(f"Could not find all {CHAPTER_COUNT} Wild Robot Escapes chapters in order")
    ...
    for index, (number, start_page, title) in enumerate(starts):
        ...
        chapters.append({
            "id": f"wild-robot-2-chapter-{number}",
            "number": number,
            "title": title,
            "startPage": start_page,
            "endPage": end_page,
            "testId": f"wild-robot-2-chapter-{number}",
        })
    ...
    OUTPUT.write_text(
        "export const WILD_ROBOT_2_META = "
