...
function renderLocked(message){
  ...
  gate.innerHTML=`<div class="arcade-access-card"><img src="assets/dragonswood-arcade-crest.svg" alt=""><h1>Arcade Time</h1>${tokens(count)}<p>${esc(message)}</p><p class="arcade-access-note">3 Tokens = one 30-minute session. This isolated preview uses browser-only state and cannot write to Firebase.</p><div class="arcade-access-actions"><button type="button" data-start-arcade ${ready?'':'disabled'}>Use 3 Tokens — Start 30 Minutes</button><a href="../v33-integration/student-manual-preview.html#arcade">Return to Dragonswood</a></div><p class="arcade-access-note">${enabled?'Teacher Arcade Time is open.':'Teacher Arcade Time is currently locked.'} • LOCAL MANUAL PREVIEW</p></div>`;
