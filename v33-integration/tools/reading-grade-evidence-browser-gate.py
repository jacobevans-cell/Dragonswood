...
def main():
    ...
    try:
      with sync_playwright() as pw:
        ...
        teacher.get_by_role('button',name='Open Requests').click();teacher.get_by_text('Please keep the useful teacher tools',exact=False).wait_for();teacher.get_by_label('Status').select_option('planned');teacher.get_by_label('Message shown to student').fill('Kept in the current V3 portal.');teacher.get_by_label('Private teacher note').fill('V57.1.8 migration browser proof.');teacher.get_by_role('button',name='Save review').click();teacher.get_by_role('dialog').wait_for(state='detached')
        ...
        teacher.get_by_role('button',name='Manage AI').click();teacher.get_by_label('Answer Rescue').select_option('true');teacher.get_by_label('Calls per student/day').fill('13');teacher.get_by_label('Calls for class/day').fill('260');teacher.get_by_role('button',name='Save AI settings').click();teacher.get_by_role('dialog').wait_for(state='detached')
