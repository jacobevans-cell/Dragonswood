// Supplied pet motion and complete transparent sprites, adapted to the battle target and lifecycle.
import {
  MOTIONS,
  clamp,
  durationFor,
  poseAt,
  validateDefinition,
} from "./pet-motion.js?v=dragon-path-11";
import { drawEnemyEffects } from "./enemy-effects.js?v=dragon-path-11";
import { enemyFacing } from "./battle-geometry.js?v=dragon-path-11";
...
async function acquire(scene, url, bounds) {
  ...
  if (!entry) {
    entry = { key: `dw-pet-${++serial}`, refs: 0 };
    ...
    entry.ready = new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(
        () => finish(new Error("Pet image timeout")),
        8000,
      );
      img.onload = () => finish();
      img.onerror = () =>
        finish(new Error(`Pet image could not load: ${url}`));
      img.src = url;
    }).then((img) => {
      if (
        !scene.sys?.settings ||
        ...
        throw new Error("Pet scene closed while loading");
      if (
...
/** Load one complete pet sprite. Play animation without a separate frame sheet. */
export async function createPetActor(scene, definition, options = {}) {
  const view = options.view ?? "back";
  if (!definition.views?.[view]) throw new Error("Missing companion view");
  definition = {...definition, ...definition.views[view], view};
  ...
  if (!scene.sys?.settings || !scene.sys.isActive()) {
    asset.release();
    throw new Error("Pet scene is no longer active");
  }
  return new PetActor(scene, definition, asset, options);
}
class PetActor {
  constructor(scene, definition, asset, options) {
    ...
    this.scale = Math.min(
      (options.height ?? 120) / b.height,
      (options.maxWidth ?? 128) / b.width,
    );
  ...
  play(
    ...
    if (this.disposed) throw new Error("Pet actor was disposed");
    if (!MOTIONS.includes(motion))
      throw new Error(`Unknown pet motion: ${motion}`);
    this.motion = motion;
    this.loop = loop;
    this.elapsed = 0;
    this.impactSent = false;
