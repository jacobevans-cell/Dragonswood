(function(){'use strict';
 ...
 window.addEventListener('message',event=>{const frame=document.querySelector('[data-dragon-path-frame]');if(!frame||event.source!==frame.contentWindow||event.origin!==location.origin)return;const data=event.data;if(!data||typeof data!=='object')return;
  if(data.type==='dragonswood-path-view'&&allowed.has(data.route)){if(Number.isFinite(data.height))frame.style.height=Math.min(30000,Math.max(420,data.height+4))+'px';const current=route();if(current&&current!==data.route){const hash=data.route==='home'?'#missions':data.route==='adventurer'?'#hall':data.route==='schedule'?'#day':'#missions/'+data.route;history.replaceState(null,'',portalUrl(hash));frame.dataset.parentHash=hash;}}
