...
const state = {
  ...
  day: 'Today',
  characterClass: storageGet('class','Warrior'),
  pet: storageGet('pet','Nyx'),
  bossHp: 72,
  bossMax: 100,
  bossMessage: ''
};
...
function navMarkup(){
  return `
    ...
    <button class="signout" type="button" data-toast="Sign out is disabled in this safe prototype.">↪ Sign out</button>`;
}
...
function welcome(){return `<div class="welcome-strip"><div>✦ &nbsp;<b>Good morning, Jacob!</b> &nbsp;<span>Your next win is ready.</span></div><button class="btn btn-secondary btn-sm" data-read>🔊 Read this page</button></div>`}
...
function adventurePage(){
  const pct=Math.round(state.xp/state.xpMax*100);
  ...
  <section class="adventure-grid">
    <article class="panel adventurer-card">
      <div class="adventurer-art"><img src="assets/art/adventurer-art.jpg" alt="Jacob the Dragon Keeper with active pet Nyx"></div>
      <div class="adventurer-info">
        <span class="rarity-chip">✦ EPIC ADVENTURER</span>
        <h2>Jacob the Dragon Keeper</h2><p>Grade 5 • ${state.characterClass} Class</p>
...
function bind(){
  ...
  app.querySelectorAll('[data-class]').forEach(el=>el.addEventListener('click',()=>{state.characterClass=el.dataset.class;storageSet('class',state.characterClass);showToast(`${state.characterClass} class equipped.`);render()}));
  app.querySelectorAll('[data-pet]').forEach(el=>el.addEventListener('click',()=>{state.pet=el.dataset.pet;storageSet('pet',state.pet);showToast(`${state.pet} is now your active companion.`);render()}));
