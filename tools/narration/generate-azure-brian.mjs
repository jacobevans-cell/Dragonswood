...
const pending=[];
for(const clip of planned){
  const old=previous.clips?.[clip.id],unchanged=!force&&old?.voiceHashes?.[VOICE_ID]===clip.hash&&old?.locale===clip.locale&&old?.sources?.[VOICE_ID]===clip.source&&await exists(clip.output);
...
for(const clip of planned){
  ...
  manifest.clips[clip.id]={hash:clip.hash,defaultVoice:VOICE_ID,contentType:clip.contentType,locale:clip.locale,sources:{[VOICE_ID]:clip.source},voiceHashes:{[VOICE_ID]:clip.hash},voiceVersion:1,textVersion:clip.textVersion,fallback:clip.fallback,audioSha256:clip.audioSha256||old?.audioSha256||null};
}
