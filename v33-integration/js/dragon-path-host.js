(function(){'use strict';
 ...
 function route(hash=location.hash){const value=String(hash).replace(/^#/,'');if(!value||['missions','home'].includes(value))return 'home';if(['adventure','hall'].includes(value))return 'adventurer';if(value==='day')return 'schedule';if(value.startsWith('missions/')&&allowed.has(value.slice(9)))return value.slice(9);if(allowed.has(value))return value;return null;}
 function markup(){const url=new URL('../learning-portal/public/embedded.html',document.baseURI);url.searchParams.set('embedded','1');url.searchParams.set('v','dragon-path-2');url.hash=route()||'home';return `<section class="unified-dragon-path"><iframe data-dragon-path-frame src="${url.href}" title="Dragon’s Path — daily lessons and adventurer" style="width:100%;min-height:640px;border:0;display:block" scrolling="no"></iframe></section>`;}
 ...
 window.addEventListener('message',event=>{const frame=document.querySelector('[data-dragon-path-frame]');if(!frame||event.source!==frame.contentWindow||event.origin!==location.origin)return;const data=event.data;if(!data||typeof data!=='object')return;
  if(data.type==='dragonswood-path-view'&&allowed.has(data.route)){if(Number.isFinite(data.height))frame.style.height=Math.min(30000,Math.max(420,data.height+4))+'px';const current=route();if(current&&current!==data.route){const hash=data.route==='home'?'#missions':data.route==='adventurer'?'#adventure':data.route==='schedule'?'#day':'#missions/'+data.route;history.replaceState(null,'',hash);frame.dataset.parentHash=hash;}}
