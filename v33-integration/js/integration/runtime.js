(function(){
  ...
  async function startTeacher(onUpdate){
    ...
    const authUnsub=S.auth.onAuthStateChanged(auth,user=>{
      ...
      [['activeWritingSession','scribe'],['activePoll','activePoll'],['activeTeacherAttention','activeAttention'],['kingdomAccess','kingdomAccess'],['dailyAccessOverride','dailyOverride'],['passBlackout','passBlackout'],['main','classMain'],['secondRecess','secondRecess'],['classPet','classPet'],['fieldTrip','fieldTrip'],['universalPoints','universalPoints'],['classJobs','classJobs'],['classSchedule','classSchedule']].forEach(([id,key])=>watchDoc(id,key));
    });
    ...
    const controller={environment,async signIn(){const provider=new S.auth.GoogleAuthProvider();return S.auth.signInWithPopup(auth,provider)},async signOut(){return S.auth.signOut(auth)},
      ...
      async sendAttention(input={}){requireWrite();const message=String(input.message||'').trim().slice(0,1200);if(!message)throw new Error('Enter a direction for students.');const allowed=new Set(['missions','module/daily-quest','module/curriculum-quest','day','adventure']),destination=allowed.has(String(input.destination||''))?String(input.destination):'missions',id=`attention_${Date.now()}`,payload={id,active:true,dateKey:today(),title:String(input.title||'Teacher Direction').trim().slice(0,120)||'Teacher Direction',message,destination,requireAcknowledgment:true,createdAtMs:Date.now(),createdBy:currentUser.uid,createdAt:S.firestore.serverTimestamp(),updatedAt:S.firestore.serverTimestamp()};const batch=S.firestore.writeBatch(db);batch.set(S.firestore.doc(db,'classData','activeTeacherAttention'),payload,{merge:false});batch.set(S.firestore.doc(db,'teacherAttentionEvents',`${id}_sent`),{attentionId:id,type:'sent',dateKey:today(),title:payload.title,message,destination,studentId:'',studentName:'Whole class',createdBy:currentUser.uid,createdAt:S.firestore.serverTimestamp(),updatedAt:S.firestore.serverTimestamp()});await batch.commit();return id},
