(function(root,factory){
  ...
  function studentPasses(uid,dateKey,input={}){
    ...
    for(const [type,def] of Object.entries(TYPES)){
      ...
      rows[type]=Object.freeze({type,label:def.label,icon:def.icon,automatic:def.automatic,used,credits,active,action,message,requestPending:requests[type]?.status==='pending'});
    }
    return Object.freeze({dateKey,group,blackout,pendingType:pending?.[0]||'',rows:Object.freeze(rows)});
  }

  return Object.freeze({TYPES,definition,statusId,requestId,bathroomGroup,studentPasses});
});
