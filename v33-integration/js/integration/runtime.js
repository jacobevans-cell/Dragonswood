(function(){
  ...
  async function startStudent(onUpdate){
    ...
    const {S,auth,db}=F;
    let profileUnsub=null,dailyUnsub=null,overrideUnsub=null,scribeUnsub=null,responsesUnsub=null,gamesUnsub=null,scheduleUnsub=null,jobsUnsub=null,eventsUnsub=null,jobWeekUnsub=null,scoresUnsub=null,rewardsUnsub=null,lootUnsub=null,prizesUnsub=null,pollUnsub=null,pollVotesUnsub=null,attentionUnsub=null,attentionEventsUnsub=null,kingdomAccessUnsub=null;
    ...
    let lastProfile=null,lastDaily=[],lastOverride={},lastScribe={},lastResponses=[],lastGames=[],lastSchedule={},lastJobs={},lastEvents=[],lastJobWeek=null,lastScores=[],lastRewards=[],lastLoot=[],lastPrizes=[],lastPoll={},lastPollVotes=[],lastAttention={},lastAttentionEvents=[],lastKingdomAccess={},currentUser=null,testerAuthorized=false;
    let lastPassStatuses={},lastPassRequests={},lastBathroomSlots={},lastPassBlackout={};
    const ready={profile:false,daily:false,override:false,scribe:false,responses:false,games:false,schedule:false,jobs:false,events:false,jobWeek:false,scores:false,rewards:false,loot:false,prizes:false,poll:false,pollVotes:false,attention:false,attentionEvents:false,kingdomAccess:false,bathroomStatus:false,snackStatus:false,outOfSeatStatus:false,officeStatus:false,bathroomRequest:false,snackRequest:false,outOfSeatRequest:false,officeRequest:false,boySlot:false,girlSlot:false,passBlackout:false};
    const clear=()=>{
      for(const fn of [profileUnsub,dailyUnsub,overrideUnsub,scribeUnsub,responsesUnsub,gamesUnsub,scheduleUnsub,jobsUnsub,eventsUnsub,jobWeekUnsub,scoresUnsub,rewardsUnsub,lootUnsub,prizesUnsub,pollUnsub,pollVotesUnsub,attentionUnsub,attentionEventsUnsub,kingdomAccessUnsub,bathroomStatusUnsub,snackStatusUnsub,outOfSeatStatusUnsub,officeStatusUnsub,bathroomRequestUnsub,snackRequestUnsub,outOfSeatRequestUnsub,officeRequestUnsub,boySlotUnsub,girlSlotUnsub,blackoutUnsub])try{fn?.()}catch{}
      profileUnsub=dailyUnsub=overrideUnsub=scribeUnsub=responsesUnsub=gamesUnsub=scheduleUnsub=jobsUnsub=eventsUnsub=jobWeekUnsub=scoresUnsub=rewardsUnsub=lootUnsub=prizesUnsub=pollUnsub=pollVotesUnsub=attentionUnsub=attentionEventsUnsub=kingdomAccessUnsub=null;
      ...
      lastProfile=null;lastDaily=[];lastOverride={};lastScribe={};lastResponses=[];lastGames=[];lastSchedule={};lastJobs={};lastEvents=[];lastJobWeek=null;lastScores=[];lastRewards=[];lastLoot=[];lastPrizes=[];lastPoll={};lastPollVotes=[];lastAttention={};lastAttentionEvents=[];lastKingdomAccess={};
    ...
    const push=()=>{
      ...
      const kingdom=Operations.datedFeatureAccess(lastKingdomAccess),attention=Operations.attentionModel(lastAttention,lastAttentionEvents,[{id:currentUser.uid,name:lastProfile?.firstName||currentUser.displayName||'Scholar'}]);
      emit(onUpdate,{status:'authorized',user:currentUser,
        ...
        kingdomAccess:Object.freeze({...kingdom,unlocked:kingdom.active&&(kingdom.all||kingdom.studentIds.includes(currentUser.uid))})});
    };
  ...
  async function startTeacher(onUpdate){
    ...
    const push=()=>{if(!currentUser||!Object.values(ready).every(Boolean))return;const rawById=new Map(data.roster.map(row=>[row.id,row])),students=Core.normalizeTeacherRoster(data.roster).map(row=>{const raw=rawById.get(row.id)||{};return {...row,hp:Number(raw.hp)||0,xp:Number(raw.xp)||0,gold:Number(raw.gold)||0,dailyXpEarned:Number(raw.dailyXpEarned)||0,optionalAccessPaused:raw.optionalAccessPaused===true,teacherCheckInRequired:raw.teacherCheckInRequired===true,reflectionRequired:raw.reflectionRequired===true,teacherFlag:String(raw.teacherFlag||''),title:String(raw.title||''),classId:String(raw.classId||''),activePet:String(raw.activePet||'')}});lastOperations=Operations.teacherOperations({students,requests:{bathroomRequests:data.bathroomRequests,snackRequests:data.snackRequests,passRequests:data.passRequests},statuses:{bathroomStatus:data.bathroomStatus,snackStatus:data.snackStatus,passStatus:data.passStatus},pointRequests:data.pointRequests,passHistory:data.passHistory,bathroomSlots:data.bathroomSlots,passBlackout:data.passBlackout,curriculumOverrides:data.curriculumOverrides,activePoll:data.activePoll,pollVotes:data.pollVotes,activeAttention:data.activeAttention,attentionEvents:data.attentionEvents,kingdomAccess:data.kingdomAccess,dailyOverride:data.dailyOverride,classData:{main:data.classMain,secondRecess:data.secondRecess,classPet:data.classPet,fieldTrip:data.fieldTrip,universalPoints:data.universalPoints},classJobs:data.classJobs,jobWeeks:data.jobWeeks,classSchedule:data.classSchedule,calendarEvents:data.calendarEvents,scores:data.scores,leaderboardRewards:data.leaderboardRewards});emit(onUpdate,{status:'authorized',user:currentUser,teacherName:currentUser.displayName||'Mr. Evans',students,academic:Academic.teacherAcademic(students,data.scribe,data.responses,data.daily,data.curriculum,data.games,data.gradeSettings),operations:lastOperations});scheduleBathroomSlotReconcile()};
    const authUnsub=S.auth.onAuthStateChanged(auth,user=>{
      ...
      [['students','roster'],['dailyQuestProgress','daily'],['curriculumAttempts','curriculum'],['writingResponses','responses'],['gameResults','games'],['bathroomRequests','bathroomRequests'],['snackRequests','snackRequests'],['passRequests','passRequests'],['pointRequests','pointRequests'],['bathroomStatus','bathroomStatus'],['snackStatus','snackStatus'],['passStatus','passStatus'],['passHistory','passHistory'],['bathroomSlots','bathroomSlots'],['curriculumOverrideRequests','curriculumOverrides'],['classPollVotes','pollVotes'],['teacherAttentionEvents','attentionEvents'],['studentJobWeeks','jobWeeks'],['classCalendarEvents','calendarEvents'],['scores','scores'],['leaderboardRewards','leaderboardRewards']].forEach(([collection,key])=>watchCollection(collection,key));
      [['activeWritingSession','scribe'],['activePoll','activePoll'],['activeTeacherAttention','activeAttention'],['kingdomAccess','kingdomAccess'],['dailyAccessOverride','dailyOverride'],['gradebookSettings','gradeSettings'],['passBlackout','passBlackout'],['main','classMain'],['secondRecess','secondRecess'],['classPet','classPet'],['fieldTrip','fieldTrip'],['universalPoints','universalPoints'],['classJobs','classJobs'],['classSchedule','classSchedule']].forEach(([id,key])=>watchDoc(id,key));
    });
    ...
    const teacherCommands=Object.freeze({
      ...
      'Dragonswood Leadership':{xp:40,gold:8,goal:'universalPoints',goalDelta:1},
    ...
    async function addGoalPoints(goalId,delta){
      const ref=S.firestore.doc(db,'classData',goalId);return S.firestore.runTransaction(db,async tx=>{const snap=await tx.get(ref),row=snap.exists()?snap.data():{},field=goalId==='universalPoints'?'points':'points',next=Math.max(0,(Number(row[field])||0)+Number(delta||0));tx.set(ref,{[field]:next,updatedAt:S.firestore.serverTimestamp()},{merge:true});return next})
    }
    const controller={environment,async signIn(){const provider=new S.auth.GoogleAuthProvider();return S.auth.signInWithPopup(auth,provider)},async signOut(){return S.auth.signOut(auth)},
      ...
      async saveGradebookWeights(input={}){requireWrite();const weights=Academic.normalizeWeights(input);if(Number(input.daily)+Number(input.curriculum)+Number(input.reading)!==100)throw new Error('Category weights must total 100%.');await S.firestore.setDoc(S.firestore.doc(db,'classData','gradebookSettings'),{...weights,updatedBy:currentUser.uid,updatedAt:S.firestore.serverTimestamp()},{merge:false});return weights},
      ...
      async adjustGoal(goalId,delta){requireWrite();if(!['secondRecess','classPet','fieldTrip'].includes(goalId))throw new Error('Unknown class goal.');return addGoalPoints(goalId,Math.max(-100,Math.min(100,Number(delta)||0)))},
