  ...
  function prepareChild(frame){
    ...
    if(/\/daily-quest\.html$/.test(new URL(frame.src,document.baseURI).pathname)){
      if(globalThis.DWMorningMathCoach?.install)globalThis.DWMorningMathCoach.install(frame);
        ...
        const coach=document.createElement('script');coach.src=new URL('js/integration/morning-math-coach.js?v=1.0.4',document.baseURI).href;coach.dataset.dwMorningMathCoachLoader='1';coach.onload=()=>globalThis.DWMorningMathCoach?.install(frame);document.head?.append(coach);
      }
    }
