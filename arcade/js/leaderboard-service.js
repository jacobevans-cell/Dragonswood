import {getFirebaseContext,getCurrentAccess} from './access-client.js';
...
export async function initLeaderboard(onStatus=()=>{}) {try{onStatus('CONNECTING…');const C=await getFirebaseContext();cloud={enabled:true,db:C.db,auth:C.auth,user:C.user,fs:C.fsMod};onStatus('CLOUD LEADERBOARD')}catch(err){console.warn('Arcade cloud leaderboard unavailable:',err);onStatus('LOCAL RECORDS')}return cloud;}
...
export async function submitBestScore(event, profile) {
  ...
  let any=false;
  if(cloud.enabled){
    const sessionId=String(getCurrentAccess()?.sessionId||'');if(!sessionId)return {updated:false,score,boardId,reason:'no-live-session'};
    const {doc,setDoc,serverTimestamp}=cloud.fs;
    const writes=periods.map(async type=>{const key=periodKey(type);const ref=doc(cloud.db,'arcadeScores',scoreDocId(uid,boardId,type,key));try{await setDoc(ref,{uid,studentKey:String(profile.studentId||'').slice(0,100),displayName:String(profile.displayName||'Adventurer').slice(0,32)||'Adventurer',gameId:String(event.gameId||boardId),boardId,periodType:type,periodKey:key,score,metric:String(event.metric??score).slice(0,100),details:event.details&&typeof event.details==='object'?event.details:{},schemaVersion:2,sessionId,updatedAt:serverTimestamp()},{merge:true});return true}catch(err){if(err?.code==='permission-denied'||String(err?.message||'').toLowerCase().includes('permission'))return false;throw err}});
