...
export function durationFor(motion,style='slash') {
  if(!MOTIONS.includes(motion))throw new Error(`Unknown pet motion: ${motion}`);
...
// One outbound arc and a lower return arc. All coordinates are stage-relative.
export function flightPoint(t,origin,area) {
  t=clamp(t);const side=origin.x>=area.center?1:-1;
  const farX=area.center-side*area.radius;
  if(t<.58){const u=smooth(t/.58);return{x:origin.x+(farX-origin.x)*u,y:origin.y+(area.midY-origin.y)*u-Math.sin(u*Math.PI)*area.lift};}
  const u=smooth((t-.58)/.42);return{x:farX+(origin.x-farX)*u,y:area.midY+(origin.y-area.midY)*u+Math.sin(u*Math.PI)*area.lift*.8};
}
export function validateDefinition(d){
  if(!d||!/^[-a-z0-9]+$/.test(d.id)||!d.name||!d.path||!ATTACKS.includes(d.attackStyle))throw new Error('Invalid pet definition');
