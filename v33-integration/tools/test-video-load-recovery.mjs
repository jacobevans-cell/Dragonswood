...
test('player publishes source candidates without changing identity or skip lock',()=>{const html=lockedVideo({id:'math-1',title:'Place Value',url:'original',fallbackUrl:'copy',durationSeconds:232},{});assert.match(html,/data-video-id="math-1"/);assert.match(html,/data-video-sources="\[&quot;copy&quot;,&quot;original&quot;\]"/);assert.match(html,/data-video-forward disabled/);assert.ok(html.indexOf('data-video-error')<html.indexOf('data-video-position'));});
