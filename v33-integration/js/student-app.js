...
function unfinishedRequiredWork(target='activity'){
  ...
  if(afternoonSubstituteActive()&&afternoonDestination(destination)){
    if(state.morningWorkComplete!==true)rows.push({id:'morning',icon:'🌅',title:'Daily Battle',detail:'Finish today’s Daily Battle.',route:'module/daily-quest'});
  ...
  if(state.dailyAccessOverride!==true){
    if(!testerMorningOverride&&state.dailyAccessUnlocked!==true)rows.push({id:'morning',icon:'🌅',title:'Daily Battle',detail:state.morningWorkComplete?'Teacher check-in or access hold remains.':'Not complete today.',route:'module/daily-quest'});
...
function attentionDestinationLabel(destination){return ({'missions':'Dragon’s Path','module/daily-quest':'Daily Battle','module/curriculum-quest':'Recovery Quest','day':'My Day','adventure':'My Adventure'})[destination]||'Dragon’s Path'}
  ...
  <section class="adventure-grid">
    <article class="panel adventurer-card">
      <div class="adventurer-art live-adventurer-stage" style="--adventure-background:url('${escapeHtml(identity.backgroundArt)}')">
        ${identity.learning?`<div class="live-adventurer-hero" data-live-adventurer>${window.DWLearningBridge.markup(identity.profile)}</div>`:identity.appearance?.v5?'<div class="live-adventurer-hero" data-live-adventurer></div>':`<img class="live-adventurer-hero" data-live-adventurer src="${escapeHtml(identity.heroArt)}" data-static-art="${escapeHtml(identity.staticHeroArt)}" alt="${escapeHtml(state.displayName)} wearing ${escapeHtml(identity.appearanceName)}">`}
    ...
    <article class="panel next-step">
      <div class="eyebrow">⭐ Your next step</div><div class="next-reward">5 points · no prizes</div><div class="next-icon">📜</div><h2>Daily Battle</h2><p>Practice yesterday’s learning, older review, personal support, and a small challenge. Visual Coach help is available.</p>
      <div class="step-pills"><div class="step-pill done">✓ Log in</div><div class="step-pill current">2 Start Daily Battle</div><div class="step-pill">3 Continue your path</div></div>
...
let learningIdentityDispose=null;
function disposeAdventureIdentity(){learningIdentityDispose?.();learningIdentityDispose=null;if(adventureMotionTimer){clearInterval(adventureMotionTimer);adventureMotionTimer=null}if(adventurePetActor){adventurePetActor.destroy();adventurePetActor=null}}
function playAdventureIdentity(stateName='play'){
  ...
  if(identity.appearance?.v5)window.DWRPG.renderV5Character(hero,identity.appearance,stateName,{prefix:'../',version:'56.33-v5.3.9',alt:identity.appearanceName});else{const animated=identity.appearance?.[`${stateName}Art`]||identity.appearance?.attackArt||identity.heroArt;hero.src=animated}hero.classList.remove('live-adventurer-action');void hero.offsetWidth;hero.classList.add('live-adventurer-action');adventurePetActor?.play(stateName,stateName==='celebrate'?1400:1000);setTimeout(()=>{if(hero.isConnected){if(identity.appearance?.v5)window.DWRPG.renderV5Character(hero,identity.appearance,'idle',{prefix:'../',version:'56.33-v5.3.9',alt:identity.appearanceName});else hero.src=identity.heroArt;hero.classList.remove('live-adventurer-action')}},1100);
}
function mountAdventureIdentity(){
  ...
  if(identity.learning){learningIdentityDispose=window.DWLearningBridge.mount(app,identity.profile);return;}
  if(identity.appearance?.v5)window.DWRPG.renderV5Character(hero,identity.appearance,'idle',{prefix:'../',version:'56.33-v5.3.9',alt:identity.appearanceName});else hero.onerror=()=>{if(hero.getAttribute('src')!==identity.staticHeroArt)hero.src=identity.staticHeroArt};
...
const missions = [
  {id:'morning',module:'daily-quest',n:'1',kicker:'DO THIS FIRST',icon:'🌅',title:'Daily Battle',desc:'25 questions: 12 math, 11 language arts, and 2 science. Coaching and saved retries help you finish.',time:'≈25 min',reward:'5 points · no prizes',button:'Start Daily Battle →'},
...
function testerControlsDialog(){
  ...
  const rows=[['unlockMorning','Daily Battle + Rune + Dragon Tongues'],['unlockCurriculum','Curriculum Quest'],['unlockArcade','Arcade'],['unlockKingdom','Kingdom Wars'],['unlockBoss','Boss Battle preview']];
...
function handleModuleState(event){
  ...
  if(message.type==='daily-mission-state'){
    ...
    // A completed placement benchmark counts as Daily Battle. Once its live
...
function arcadeBlockedMessage(access){
  ...
  if(access?.afternoonSubstituteActive===true){const requirements=access.afternoonRequirements||{},missing=[requirements.morningComplete!==true?'Daily Battle':'',requirements.curriculumComplete!==true?'every Current Quest lesson':''].filter(Boolean);return `Finish ${missing.join(' and ')} to unlock free Afternoon Arcade Time.`}
...
async function openModule(id){
  ...
  if(window.DWLearningBridge?.open(id))return;
...
function mountModule(id){if(id==='boss-battle'){location.hash='boss';return;}if(window.DWLearningBridge?.open(id))return;if(id)moduleHost?.mount(app,id,document.baseURI)}
