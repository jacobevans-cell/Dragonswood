...
async function fixture(run,{stallMedia=false}={}){
 const old={document:globalThis.document,window:globalThis.window,setInterval,clearInterval};
 const video=Object.assign(new Element(),{paused:true,currentTime:0,duration:600,readyState:stallMedia?0:1,error:null,playbackRate:1.5,getAttribute:()=> 'copy',load(){},pause(){this.paused=true;},async play(){this.paused=false;}});
 ...
 let tick,interval,held=null,delay=false,failOnce=false,failStatus,startCount=0;const packets=[],progress={seconds:0,position:0,frontier:0,requiredSeconds:540,durationSeconds:600,complete:false};
 globalThis.setInterval=(fn,ms)=>{tick=fn;interval=ms;return 1;};globalThis.clearInterval=()=>{};
 const api=async(path,payload)=>{
  if(path.endsWith('/start')){startCount++;return {token:'fixture-token',sequence:0,progress:{...progress}};}
  packets.push({...payload});if(failOnce){failOnce=false;throw Object.assign(new Error('Temporary disconnect'),{status:failStatus});}
 ...
 try{const opening=get('[data-video-play]').emit('click');if(stallMedia){await drain();assert.equal(startCount,0,'a stalled media download must not start the viewing clock');video.readyState=1;await video.emit('loadedmetadata');}await opening;assert.equal(startCount,1);assert.equal(video.paused,false);await run({video,get,packets,document,tick:async()=>{tick();await drain();},interval,delayNext:()=>delay=true,failNext:status=>{failOnce=true;failStatus=status;},release:async()=>{held?.();await drain();}});}
...
test('slow metadata starts no viewing lease until the media is ready',()=>fixture(async h=>{assert.equal(h.packets[0].event,'play');},{stallMedia:true}));
test('a gateway 503 retries the identical video receipt before pausing the lesson',()=>fixture(async h=>{
 h.failNext(503);h.video.currentTime=7.5;await h.tick();assert.deepEqual(h.packets[1],h.packets[2]);assert.equal(h.video.paused,false);
}));
