(function(root){
  ...
  function install(frame){
    ...
    function currentOperationProblem(){
      const prompt=String(doc.getElementById('currentPrompt')?.textContent||'').replaceAll(',','').trim();if(!prompt||/\d+\s*\/\s*\d+|\d+\.\d+/.test(prompt))return null;
      let match=prompt.match(/(\d+)\s*(\+|−|-|×|x|÷)\s*(\d+)/i),operation='';
      if(match)operation=({'+':'addition','−':'subtraction','-':'subtraction','×':'multiplication','x':'multiplication','÷':'division'})[match[2].toLowerCase()];
      if(!match){const patterns=[['addition',/(?:sum of|add)\s+(\d+)\s+(?:and|to)\s+(\d+)/i],['subtraction',/(?:difference between|subtract)\s+(\d+)\s+(?:and|from)\s+(\d+)/i],['multiplication',/(?:product of|multiply)\s+(\d+)\s+(?:and|by)\s+(\d+)/i],['division',/(?:quotient of|divide)\s+(\d+)\s+(?:and|by)\s+(\d+)/i],['division',/(\d+)\s+divided by\s+(\d+)/i],['multiplication',/(\d+)\s+times\s+(\d+)/i]];for(const [candidate,pattern] of patterns){const found=prompt.match(pattern);if(found){operation=candidate;const reverse=candidate==='subtraction'&&/^subtract/i.test(found[0]);match=[found[0],reverse?found[2]:found[1],reverse?found[1]:found[2]];break}}}
      if(!match||!operation)return null;const a=Number(match[1]),b=Number(match[3]||match[2]);if(!Number.isInteger(a)||!Number.isInteger(b)||a<0||b<0||a>99999||b>99999||(operation==='division'&&b===0))return null;return{operation,a,b,prompt};
    }
    function populateCurrentProblem(){let child;try{child=coachFrame.contentDocument}catch{return false}const problem=currentOperationProblem();if(!child||!problem)return false;child.querySelector(`[data-operation="${problem.operation}"]`)?.click();const a=child.getElementById('customA'),b=child.getElementById('customB');if(!a||!b)return false;a.value=String(problem.a);b.value=String(problem.b);child.getElementById('customNote').textContent='Loaded from the current Morning Work question. Solve it one coached step at a time; the answer remains yours.';return true}
    const close=()=>{backdrop.classList.remove('show');launch.focus()};
    launch.addEventListener('click',()=>{backdrop.classList.add('show');if(!coachFrame.src)coachFrame.src=new URL('math-operations-quest.html?dw-morning-coach=1',doc.baseURI).href;else populateCurrentProblem()});
    closeButton.addEventListener('click',close);
    coachFrame.addEventListener('load',()=>{let child;try{child=coachFrame.contentDocument}catch{return}if(!child)return;child.getElementById('customModeBtn')?.click();child.querySelector('[data-difficulty="normal"]')?.click();const compact=child.createElement('style');compact.dataset.dwMorningCoachCompact='1';compact.textContent='.dw-account,.dw-status,.round-banner,.game-hero,.difficulty-row,.difficulty-note,.top,.mission-dashboard,.mission-complete,.mode-row:first-child{display:none!important}.mode-shell{margin-top:8px!important}.mode-row-spaced{margin-top:0!important}.layout{margin-top:10px!important}main{padding-top:4px!important}.op-btn[data-operation="mixed"]{display:none!important}';child.head?.append(compact);populateCurrentProblem()});
