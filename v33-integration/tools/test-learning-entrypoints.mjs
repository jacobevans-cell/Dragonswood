...
test('learning entry points return to the replacement at the existing GitHub address',()=>{
 for(const [id,route] of [['daily-quest','missions/morning'],['curriculum-quest','missions']]){assert.equal(bridge.href(id),'https://jacobevans-cell.github.io/Dragonswood/#'+route);assert.equal(bridge.open(id),true);}
 assert.equal(bridge.open('boss-battle'),false);assert.equal(assigned.length,2);
});
test('new character identity preserves level one and only equips an owned eligible companion',()=>{
 ...
 assert.equal(p.needsClassSelection,true,'previous chooser completion must reopen exactly once');
 const complete={...raw,genderGroup:'boy',learningAdventurer:{...raw.learningAdventurer,setupVersion:'dragonswood-adventurer-setup.2'}};
 assert.equal(bridge.profileFor(complete).needsClassSelection,false);
 assert.equal(bridge.profileFor(complete).assignedGender,'male');
 assert.equal(bridge.profileFor({...complete,genderGroup:'girl'}).assignedGender,'female');
...
test('direct legacy URLs reach their replacement and retired Boss code cannot execute or award loot',()=>{
 assert.match(read('curriculum-quest.html'),/#missions/);assert.match(read('adventurer-hall.html'),/id="pets"/);assert.doesNotMatch(read('adventurer-hall.html'),/http-equiv="refresh"/);assert.equal(bridge.href('adventurer-hall'),null);
...
test('the main student and teacher shells load the new identity bridge and the student pass guard remains before learning navigation',()=>{
 ...
 assert.match(read('index.html'),/portal-entry.js/);assert.match(read('index.html'),/dragon-path-host.js/);
 ...
 assert.match(read('v33-integration/js/teacher-app.js'),/https:\/\/jacobevans-cell.github.io\/Dragonswood\/#missions\/teacher/);
});
