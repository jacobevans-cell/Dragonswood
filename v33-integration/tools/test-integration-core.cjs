...
assert.deepEqual(C.dailyMissionState(rows,new Date('2026-08-25T18:00:00Z')),{dateKey:'2026-08-25',morning:'complete',exit:'not_started'});
rows.push({id:'e_v48',status:'in_progress',session:'exit',dateKey:'2026-08-25'});
assert.equal(C.dailyMissionState(rows,new Date('2026-08-25T18:00:00Z')).exit,'in_progress');
...
assert.equal(p.dailyMissions.morning,'complete');assert.equal(p.dailyMissions.exit,'in_progress');
