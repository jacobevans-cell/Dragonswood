...
const tests = Object.values(WILD_ROBOT_3_TESTS).sort((a, b) => a.chapterNumber - b.chapterNumber);
const answerTotals = [0, 0, 0, 0];

assert.equal(WILD_ROBOT_3_META.pages, 236, "PDF edition page count changed");
assert.equal(WILD_ROBOT_3_META.storyEndPage, 230, "Story ending page changed");
assert.equal(WILD_ROBOT_3_META.chapters.length, 80, "Book 3 must have 80 chapter gates");
assert.equal(tests.length, 80, "Every chapter needs one assessment");
assert.equal(CHAPTER_MAPS["wild-robot-3"].chapters.length, 80, "Chapter map is not registered");

for (let index = 0; index < WILD_ROBOT_3_META.chapters.length; index += 1) {
  const chapter = WILD_ROBOT_3_META.chapters[index];
  ...
  if (index > 0) {
    assert.equal(chapter.startPage, WILD_ROBOT_3_META.chapters[index - 1].endPage + 1, `Gap before chapter ${chapter.number}`);
  }
}
assert.equal(WILD_ROBOT_3_META.chapters[0].startPage, 11, "Chapter 1 start page changed");
assert.equal(WILD_ROBOT_3_META.chapters.at(-1).endPage, 230, "Final test gate must precede publisher extras");

for (const test of tests) {
  const chapter = WILD_ROBOT_3_META.chapters[test.chapterNumber - 1];
...
assert.ok(Object.keys(STARTER_TESTS).length >= 901, "The Wild Robot Protects assessments are missing from the full library");
...
assert.equal(series[2].pages, WILD_ROBOT_3_META.pages, "Catalog and PDF metadata disagree");
assert.equal(series[2].file, "class-library/books/wild-robot-3-protects.pdf", "Original illustrated PDF is not retained");

console.log(JSON.stringify({
  book: "The Wild Robot Protects",
  chapters: tests.length,
  questions: tests.reduce((total, test) => total + test.questions.length, 0),
  sourcePages: WILD_ROBOT_3_META.pages,
  storyEndPage: WILD_ROBOT_3_META.storyEndPage,
