...
assert.match(teacher,/<select id="arcade-period">/,'Arcade period must be a teacher-readable select');
assert.doesNotMatch(teacher,/<label for="arcade-period">Period ID<\/label>/,'raw Period ID must not be visible');
